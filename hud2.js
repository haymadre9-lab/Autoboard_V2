...
export function createHud2(canvas, opts = {}){
  ...
  const cfgOpt = Object.assign({
    ...
    rotondaInvertida: false, // el lado por defecto ya es el correcto en España
    // Prioridad a la suavidad: la exactitud la da el mapa, no el HUD. Con estos
    // valores la escena nunca da tirones, aunque quede algo desfasada.
    correccion: 0.6,        // m/s maximos de correccion de posicion por GPS
    zonaMuerta: 10,         // errores menores que esto (m) se ignoran
    giroMax: 35,            // grados/s que puede girar la camara como maximo
    ...
    estilo: 'suave',        // 'suave' = trazado interpretado; 'real' = calcado
    radioMin: 130,          // en modo suave, ninguna curva baja de este radio (m)
    rbSmooth: 3,            // pasadas de suavizado del rumbo de toda la ruta
  ...
  const api = {};
  api.onError = null;
  api.version = '2026.09.03-2';   // sube al cambiar: sirve para saber qué está corriendo
  let carImg = null, carAR = 1;
  let manOver = null, limitOver = null, radarOver = null, streetOver = null;
  let fuera = 0, finAvisado = false, sErr = 0;
  api.onFinRuta = null;
  /** true cuando ya has pasado del final de la ruta cargada. */
  api.finDeRuta = () => !!pts && s > routeLen - 30;
  ...
  // Ventana de 12 m: con +-3.5 m un vertice suelto disparaba la curvatura y con
  // ella el segmento adaptativo, el limite de offset y la camara.
  ...
  function xsecUno(k){
    const hit = xsCache.get(k);
    if (hit !== undefined) return hit;
    const v = xsecCalc(k*4);
    if (xsCache.size > 6000) xsCache.clear();
    xsCache.set(k, v);
    return v;
  }
  // Interpolado entre celdas: antes el perfil saltaba de golpe cada 4 m y el eje
  // del coche se movia a escalones.
  function xsec(t){
    const q = t/4, k = Math.floor(q), f = q - k;
    if (f < 0.001) return xsecUno(k);
    const a = xsecUno(k), b = xsecUno(k+1);
    return { f: a.f + (b.f-a.f)*f, rf: a.rf + (b.rf-a.rf)*f,
             med: a.med + (b.med-a.med)*f, half: a.half + (b.half-a.half)*f,
             mine: a.mine + (b.mine-a.mine)*f };
  }
  function xsecCalc(t){
  ...
  api.setRoute = function(latlngs, o = {}){
    ...
    // Suavizado del rumbo con ventana de 5 puntos (10 m). Los latigazos venian
    // de vertices casi coincidentes en el polyline de TomTom: un par de puntos
    // juntos crean un pico de curvatura que la camara persigue de golpe.
    for (let p = 0; p < cfgOpt.rbSmooth; p++)
      for (let i = 2; i < N-2; i++)
        pts[i*3+2] = pts[i*3+2]*0.36
                   + (pts[(i-1)*3+2] + pts[(i+1)*3+2])*0.22
                   + (pts[(i-2)*3+2] + pts[(i+2)*3+2])*0.10;
    ...
    MW = o.motorway || [];
    RB = detectRoundabouts();          // se detectan sobre el trazado REAL
    if (cfgOpt.estilo !== 'real') estilizar();

    manList = []; limList = []; radList = []; xsCache.clear();
    fuera = 0; finAvisado = false; sErr = 0; cam0.off = null; cam0.yaw = null;
    s = Math.min(6, routeLen*0.02);
    return { metros: Math.round(routeLen), puntos: N, rotondas: RB.length };
  };

  /**
   * Reescribe el trazado para que sea AGRADABLE, no exacto. Dos reglas:
   *  1. Ninguna curva baja del radio minimo: las curvas encadenadas y cerradas
   *     se funden en una sola curva amplia, y se acaban los latigazos.
   *  2. Cada rotonda se sustituye por un gesto simple segun cuanto giras en
   *     ella: poco -> curva a la derecha, medio -> practicamente recto, mucho
   *     -> curva a la izquierda. Es lo que el conductor necesita saber.
   * Despues se reintegran las posiciones desde los nuevos rumbos: la carretera
   * deja de coincidir metro a metro con la real, y eso es intencionado.
   */
  function estilizar(){
    const kMax = 1/(cfgOpt.radioMin || 130);
    const h = new Float64Array(N);
    for (let i = 0; i < N; i++) h[i] = pts[i*3+2];

    // gesto de rotonda
    for (const r of RB){
      const i0 = Math.max(1, Math.round(r.s0/STEP)), i1 = Math.min(N-1, Math.round(r.s1/STEP));
      const gr = r.giro*180/Math.PI;
      const dir = gr < 130 ? 1 : gr < 210 ? 0 : -1;      // 1a / 2a / 3a salida
      r.gesto = dir;
      const total = dir * Math.min(gr, 75) * Math.PI/180;  // giro suave equivalente
      const n = Math.max(1, i1-i0);
      for (let i = i0; i <= i1; i++) h[i] = h[i0-1] + total*((i-i0+1)/n);
      for (let i = i1+1; i < N; i++) h[i] = h[i]; // el resto se recoloca abajo
      const desfase = h[i1] - pts[i1*3+2];
      for (let i = i1+1; i < N; i++) h[i] = pts[i*3+2] + desfase;
    }

    // limite de curvatura: nada mas cerrado que radioMin
    const dMax = kMax*STEP;
    for (let i = 1; i < N; i++){
      let d = h[i] - h[i-1];
      if (d >  dMax) d =  dMax;
      if (d < -dMax) d = -dMax;
    ...
    // suavizado final generoso
    for (let p = 0; p < 6; p++)
      for (let i = 2; i < N-2; i++)
        h[i] = h[i]*0.34 + (h[i-1]+h[i+1])*0.23 + (h[i-2]+h[i+2])*0.10;

    // reintegrar posiciones desde los rumbos nuevos
    let x = pts[0], z = pts[1];
    for (let i = 0; i < N; i++){
      pts[i*3] = x; pts[i*3+1] = z; pts[i*3+2] = h[i];
      x += Math.sin(h[i])*STEP; z += Math.cos(h[i])*STEP;
    }
    xsCache.clear();
  }
  ...
  function detectRoundabouts(){
    ...
    for (const r of runs){
      ...
      // El centro de la rotonda esta en el lado CONTRARIO al signo de curvatura
      // tal y como lo devuelve curv() con este convenio de rumbo. Verificado en
      // carretera: en España la isla queda a tu izquierda y asi sale bien.
      const sg = -r.sign * (cfgOpt.rotondaInvertida ? -1 : 1);
      out.push({ s0:r.s0, s1:r.s1, R, giro,
                 cx: p.x + n.x*R*sg, cz: p.z + n.z*R*sg });
    }
    return out;
  }
  ...
  api.syncPosition = function(lat, lng, windowM = 120){
    ...
    if (s > routeLen - 5) return null;      // ya fuera del trazado cargado
    ...
    // NO se toca la posicion aqui. Antes se corregia hasta 6 m de golpe con cada
    // fix, o sea un tiron por segundo: como la geometria depende de la posicion,
    // ese salto metia y sacaba de la curva. Ahora solo se anota el error y se
    // reparte a lo largo del tiempo, a un ritmo maximo en m/s.
    const err = gpsS - s;
    sErr = Math.abs(err) < (cfgOpt.zonaMuerta || 4) ? 0 : err;
    return { s: gpsS, desvio: desvio, error: err, fuera: false };
  };
  ...
  function setupCamera(){
    ...
    if (cam0.off === null) cam0.off = offObj;
      ...
      const dmax = 1.1*dtCam;   // 1,1 m/s: el coche no se desplaza a tirones
    ...
    if (cam0.yaw === null){ cam0.yaw = yawObj; cam0.pit = pitObj; }
      ...
      cam0.yaw += clamp(dY * Math.min(dtCam*2.6, 1), -maxY, maxY);
  ...
  function drawSky(pal){
    ...
    if (cfgOpt.ambiente){
      // MONTES AL FONDO. Dos siluetas con perfil generado por senos, desplazadas
      // con el rumbo de la camara: dan profundidad y, al moverse despacio con el
      // giro, refuerzan la sensacion de que el mundo gira y el coche no.
      const rumbo = Math.atan2(fwd.x, fwd.z);
      const gr = hexToRgb(pal.ground), sk = hexToRgb(pal.skyBot);
      for (const capa of [[0.55, 0.052, 1.0], [0.30, 0.085, 1.7]]){
        const mez = capa[0], amp = capa[1]*H, vel = capa[2];
        ctx.fillStyle = mixc(gr, sk, mez);
        ctx.beginPath(); ctx.moveTo(0, y+2);
        for (let px = 0; px <= W; px += 14){
          const u = px/W*6.2 + rumbo*vel*2.4;
          const alt = (Math.sin(u) + Math.sin(u*2.3+1.1)*0.5 + Math.sin(u*0.6+2.2)*0.8)/2.3;
          ctx.lineTo(px, y + 2 - (0.45 + alt*0.55)*amp);
        }
        ctx.lineTo(W, y+2); ctx.closePath(); ctx.fill();
      }

      // Sol o luna, segun el tema, con su halo.
      const disc = pal.glow >= 1 ? ['235,240,255', 0.55] : pal.glow > 0 ? ['255,190,120', 0.85] : ['255,246,214', 0.7];
      const dx = W*0.5 + Math.sin(rumbo*1.0 + 0.8)*W*0.42;
      const dy = y - H*0.16;
      const hg = ctx.createRadialGradient(dx, dy, 0, dx, dy, H*0.22);
      hg.addColorStop(0, `rgba(${disc[0]},${0.30*disc[1]})`);
      hg.addColorStop(1, `rgba(${disc[0]},0)`);
      ctx.fillStyle = hg; ctx.beginPath(); ctx.arc(dx, dy, H*0.22, 0, 6.29); ctx.fill();
      ctx.fillStyle = `rgba(${disc[0]},${disc[1]})`;
      ctx.beginPath(); ctx.arc(dx, dy, H*0.028, 0, 6.29); ctx.fill();
    }
  ...
  // Senal de rotonda en el arcen IZQUIERDO: disco azul con la flecha del gesto.
  // Sustituye a la isla cuando el trazado va estilizado.
  function senalRotonda(r, pal, fogRGB){
    const sM = Math.max(r.s0 - 40, s - 5);
    const cx = xsec(sM), o = -(cx.half + 1.5);
    const yb = 1.15, rad = 0.62;
    poly([ptOff(sM, o-0.07, 0), ptOff(sM+0.3, o-0.07, 0),
          ptOff(sM+0.3, o+0.07, yb), ptOff(sM, o+0.07, yb)],
         mixc([92,98,104], fogRGB, 0.1));
    const disco = [];
    for (let k = 0; k < 16; k++){ const a = k/16*6.2832;
      disco.push(ptOff(sM, o + Math.cos(a)*rad, yb + rad + Math.sin(a)*rad)); }
    poly(disco, mixc([236,238,234], fogRGB, 0.08));
    const azul = [];
    for (let k = 0; k < 16; k++){ const a = k/16*6.2832;
      azul.push(ptOff(sM-0.01, o + Math.cos(a)*rad*0.78, yb + rad + Math.sin(a)*rad*0.78)); }
    poly(azul, mixc([26,72,150], fogRGB, 0.08));
    // flecha: derecha, recta o izquierda segun el gesto calculado
    const g = r.gesto === undefined ? 0 : r.gesto;
    const fx = g*0.30;
    poly([ptOff(sM-0.02, o-0.07, yb+rad-0.34), ptOff(sM-0.02, o+0.07, yb+rad-0.34),
          ptOff(sM-0.02, o+0.07+fx, yb+rad+0.10), ptOff(sM-0.02, o-0.07+fx, yb+rad+0.10)],
         'rgba(240,246,255,0.95)');
    poly([ptOff(sM-0.03, o-0.20+fx, yb+rad+0.06), ptOff(sM-0.03, o+0.20+fx, yb+rad+0.06),
          ptOff(sM-0.03, o+fx, yb+rad+0.34)], 'rgba(240,246,255,0.95)');
  }
  ...
  function drawRoad(pal){
    ...
    for (let i = pasos.length - 1; i >= 0; i--){
      ...
      if (a < -20 || b > routeLen + 5000) continue;
      ...
      // Grano del asfalto: una banda ligeramente mas clara cada 3 segmentos.
      // Con el color plano la calzada parecia carton.
      if (cfgOpt.ambiente && fog < 0.6 && ((a/SEG)|0) % 3 === 0){
        const cl = mixc([pal.asphalt[0]+9, pal.asphalt[1]+9, pal.asphalt[2]+10], fogRGB, fog);
        poly([ptOff(a,A.med), ptOff(b,B.med), ptOff(b,B.half), ptOff(a,A.half)], cl);
        poly([ptOff(a,-A.half), ptOff(b,-B.half), ptOff(b,-B.med), ptOff(a,-A.med)], cl);
      }
      ...
      if (!islandDone && rbCur && ((rbCur.s0+rbCur.s1)/2) - s > d - SEG){
        if (cfgOpt.estilo === 'real') drawIsland(rbCur, pal, fogRGB);
        else senalRotonda(rbCur, pal, fogRGB);
        islandDone = true;
      }
  ...
  function shade(base, p0, p1, p2, fog, fogRGB, boost){
    ...
    // Sin descarte de caras traseras: con el orden por profundidad las de detras
    // se pintan primero y quedan tapadas. Descartarlas dejaba agujeros por los
    // que se veia el interior y las opticas delanteras a traves de la luna.
    if (nx*0 === 1) return null;
  ...
  function drawCar(pal, tail, head, sPos, offIn, colorIn, opp){
    ...
    if (fog > 0.96) return;
    const base = hexToRgb(colorIn || cfgOpt.carColor), glass = [12,14,17];
    ...
    for (let i = 0; i < PROF.length-1; i++){
      ...
      for (const sg of [-1,1]){
        ...
        // Costados de la luneta: son montante y aleta, o sea CHAPA opaca. Iban
        // en material traslucido y por eso el trasero se veia oscuro de lado a lado.
        face([P(A[0],sg*A[1],A[2]), P(B[0],sg*B[1],B[2]), P(B[0],sg*B[3],B[4]), P(A[0],sg*A[3],A[4])], base);
      }
      if (A[3] > 0.02 || B[3] > 0.02)
        // Luna central: negra del todo, sin realce. Es el unico cristal visible.
        face([P(A[0],-A[3],A[4]), P(B[0],-B[3],B[4]), P(B[0],B[3],B[4]), P(A[0],A[3],A[4])],
             cabin ? glass : base, cabin ? 0.40 : 1);
      else
    ...
    {
      const yT = 0.62, yP = 0.335, X = 0.800, rx = 0.075, ry = 0.045;
      const negro = mixc([26, 28, 32], fogRGB, fog);
      // contorno con las cuatro esquinas achaflanadas, como el piloto
      plano([[-X+rx,yP],[X-rx,yP],[X-rx*0.35,yP+ry*0.45],[X,yP+ry],
             [X,yT-ry],[X-rx*0.35,yT-ry*0.45],[X-rx,yT],[-X+rx,yT],
             [-X+rx*0.35,yT-ry*0.45],[-X,yT-ry],[-X,yP+ry],[-X+rx*0.35,yP+ry*0.45]]
            .map(q => P(-2.362, q[0], q[1])), negro);
      for (const sg of [-1, 1])
        plano([P(-2.362, sg*X, yP+ry*0.5), P(-2.10, sg*0.868, yP+ry*0.5),
               P(-2.10, sg*0.868, yT-ry*0.5), P(-2.362, sg*X, yT-ry*0.5)], negro);
      plano([P(-2.366, -0.60, yP+0.012), P(-2.366, 0.60, yP+0.012),
             P(-2.366, 0.60, yP+0.080), P(-2.366, -0.60, yP+0.080)],
            mixc([16, 17, 20], fogRGB, fog));
    }

    // ópticas: son geometría permanente, apagadas son cristal oscuro
    for (const sg of [-1,1]){
      // Piloto con extremos redondeados: contorno de 12 puntos con las esquinas
      // achaflanadas en dos pasos, en vez de un rectangulo seco.
      {
        const x0=0.26, x1=0.80, y0=0.905, y1=1.010, rx=0.055, ry=0.030;
        plano([[x0+rx,y0],[x1-rx,y0],[x1-rx*0.35,y0+ry*0.45],[x1,y0+ry],
              [x1,y1-ry],[x1-rx*0.35,y1-ry*0.45],[x1-rx,y1],[x0+rx,y1],
              [x0+rx*0.35,y1-ry*0.45],[x0,y1-ry],[x0,y0+ry],[x0+rx*0.35,y0+ry*0.45]]
             .map(q => P(-2.360, sg*q[0], q[1])),
             mixc([74+181*tail, 14+42*tail, 18], fogRGB, fog));
      }
      const b = head > 0.2 ? [252,250,236] : [58,62,70];
      plano([P(2.360,sg*0.26,0.760), P(2.360,sg*0.74,0.760), P(2.360,sg*0.74,0.880), P(2.360,sg*0.26,0.880)],
           mixc(b, fogRGB, fog));
      if (head > 0.2) glow(P(2.375, sg*0.50, 0.820), '255,246,220', 420, 0.55*head);
    }
    if (tail > 0.2){
      plano([P(-2.358,-0.27,0.560), P(-2.358,0.27,0.560), P(-2.358,0.27,0.690), P(-2.358,-0.27,0.690)],
           mixc([200+55*tail, 202+53*tail, 196+52*tail], fogRGB, fog));
      ctx.save(); ctx.globalCompositeOperation = 'lighter';
      plano([P(-2.356,-0.27,0.560), P(-2.356,0.27,0.560), P(-2.356,0.27,0.700), P(-2.356,-0.27,0.700)],
           `rgba(255,240,205,${0.30*tail})`);
      ctx.restore();
      for (const sg of [-1,1]) glow(P(-2.355, sg*0.19, 0.720), '255,240,203', 260, 0.85*tail);
    }
    volcar();   // todo el coche, en una sola pasada ordenada por profundidad
  }
  ...
  function frame(now){
    ...
    // Pasado el final de la ruta se sigue avanzando: at() extrapola en linea
    // recta, asi que la carretera continua en vez de acabarse bajo las ruedas.
    // Se permiten 5 km de prolongacion, suficientes para cualquier sobrepaso.
    // Avance por velocidad + correccion repartida. El limite en m/s hace que el
    // mundo se recoloque despacio en vez de dar saltos: el coche queda clavado.
    let paso = speed*dt;
    if (sErr !== 0){
      const maxC = (cfgOpt.correccion || 1.2) * dt;
      const c = clamp(sErr*0.35, -maxC, maxC);
      paso += c; sErr -= c;
      if (Math.abs(sErr) < 0.05) sErr = 0;
    }
    s = clamp(s + paso, 0, routeLen + 5000);
    if (!finAvisado && s > routeLen - 30){
      finAvisado = true;
      if (api.onFinRuta) { try { api.onFinRuta(); } catch(e){} }
    }
  ...
  api.set = o => {
    ...
    const rein  = ('rotondaInvertida' in o) && o.rotondaInvertida !== cfgOpt.rotondaInvertida;
    Object.assign(cfgOpt, o);
    if (reesc){ W = 0; H = 0; resize(); }
    if ((redo || rein) && pts){ RB = detectRoundabouts(); xsCache.clear(); }
  };
